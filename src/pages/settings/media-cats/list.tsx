...
import { FormMediaCat } from '../../../components/modules/form-media-cat'

const CONFIG_PAGE_EDIT = {
    title: 'Categorias de Formato de Peça',
    name: 'Categoria de Formato de Peça',
    url: 'media-cats',
    button_new: 'Nova Categoria',
    permission_add: 'media_cats_add',
    permission_edit: 'media_cats_edit',
    permission_remove: 'media_cats_delete',
    icon_breadcrumb: <IconTag />,
    FormEdit: FormMediaCat,
}

export default function SettingsMediaCats({ addBreadCrumb }: { addBreadCrumb(icon: any, name: string, redirect: string): void }) {
    ...
    const [data, setData] = useState<IMediaCat[]>([])
    ...
    const getData = async (page: number, limit: number, search: string, order: string) => {
        setLoading(true);
        const response = await MediaService.getCats(pagination.page, pagination.limit, search, order)
    ...
    const handleSaveEdit = (item: IMediaCat) => {
        setData([...data.map((row) => {
            return row.media_cat_id === item.media_cat_id ? { ...row, ...item } : row
        })])
        addBreadCrumb(CONFIG_PAGE_EDIT.icon_breadcrumb, item.title, `/settings/${CONFIG_PAGE_EDIT.url}/${item.media_cat_id}`);
    }
    ...
    const handleDelete = async () => {
        ...
        await MediaService.deleteCat(Number(DTODelete));
        getData(pagination.page, pagination.limit, search, order);
        setDTODelete(null)
        ...
        <S.Container>
            ...
            {(id && id !== 'new') &&
                <CONFIG_PAGE_EDIT.FormEdit
                    ...
                    onLoad={(item) => addBreadCrumb(CONFIG_PAGE_EDIT.icon_breadcrumb, item.title, `/settings/${CONFIG_PAGE_EDIT.url}/${item.media_cat_id}`)}
                    id={Number(id)}
                />
            }
                ...
                <div className='list'>
                    ...
                    <TableDefault
                        ...
                        tbody={
                            <tbody>
                                {data.map((row, index) =>
                                    <RenderTD onDelete={setDTODelete} key={`td-row-${row.media_cat_id}-${index}`} row={row} />
                                )}
                            </tbody>
                        }
                    />
...
const RenderTD = ({ row, onDelete }: { row: IMediaCat, onDelete(id: number): void }) => {
    ...
    const submenu: ISubmenuSelect[] = [
        {
            name: 'Editar',
            icon: <IconPencil />,
            onClick: () => redirectSlug(`settings/${CONFIG_PAGE_EDIT.url}/${row.media_cat_id}`),
            permission: CONFIG_PAGE_EDIT.permission_edit,
        },
        {
            name: 'Remover',
            icon: <IconTrash />,
            onClick: () => onDelete(row.media_cat_id),
            permission: CONFIG_PAGE_EDIT.permission_remove
        }
    ]

    return (
        <tr key={`tr-${row.media_cat_id}`}>
            <td>
            ...
            <td>
                {row.media_cat_id !== 1 && row.media_cat_id !== 2 &&
